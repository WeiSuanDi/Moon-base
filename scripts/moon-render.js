...
let scene, camera, renderer, controls, composer, moon, markerGroup, glow, stars;
...
export function initMoon() {
  ...
  const markerMaterial = new THREE.MeshBasicMaterial({ color: 0x00d4ff, transparent: true, opacity: 0.9 });
  ...
  bases.forEach(base => {
    ...
    const dot = new THREE.Mesh(new THREE.SphereGeometry(0.022, 24, 24), markerMaterial.clone());
    dot.position.copy(pos);
    dot.userData = { base, type: 'dot' };

    const ring = new THREE.Mesh(new THREE.RingGeometry(0.035, 0.045, 48), ringMaterial.clone());
  ...
  function animate() {
    ...
    markerGroup.children.forEach(child => {
      if (child.userData.type === 'ring') {
        ...
        child.material.opacity = 0.4 - Math.sin(t * 3) * 0.2;
      }
    });
